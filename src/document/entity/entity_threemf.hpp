...
namespace dune3d {
class EntityThreeMF : public EntityT<EntityThreeMF>,
                      public IEntityNormal,
                      public IEntityMovable3D,
                      public IEntityDeletePoint {
public:
    explicit EntityThreeMF(const UUID &uu);
    explicit EntityThreeMF(const UUID &uu, const json &j, const std::filesystem::path &containing_dir);
    ...
    // Kept for future parity with EntitySTEP/EntitySTL (anchor points on
    // the imported mesh), even though no tool creates them yet.
    ...
    std::shared_ptr<const Imported3MF> m_imported;
