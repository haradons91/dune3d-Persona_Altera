#include "stl_import_manager.hpp"
...
{
    return std::filesystem::path(Glib::get_user_cache_dir()) / "dune3d" / "stl";
}
...
STLImportManager::STLImportManager()
{
    create_cache_dir();
}

STLImportManager &STLImportManager::get()
{
    static STLImportManager instance;
    return instance;
}
...
// step_import_manager.cpp -- these need to stay TU-local so the two don't
// collide with duplicate-symbol link errors.
namespace face {
...
std::shared_ptr<ImportedSTL> STLImportManager::import_stl(const std::filesystem::path &path)
{
    ...
    auto imported = std::make_shared<ImportedSTL>(path, hash);
    imported->ready = true;
    bool cache_ok = false;
    ...
    if (!cache_ok) {
        imported->result = STLImporter::import(path);
